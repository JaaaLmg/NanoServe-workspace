...
- [ ] `SequenceStatus` 包含六种计划要求的状态。
- [ ] 状态迁移图、合法迁移表和代码规则一致。
- [ ] 非法迁移有显式异常，错误信息可定位请求。
- [ ] 取消、超时、抢占、恢复和正常完成均有统一入口。
- [ ] 终态请求不再被调度，且 KV block 已释放。
- [ ] 抢占恢复不丢失 prompt 进度和已生成 token。
- [ ] 重复完成/取消/清理不会 double free 或重复入队。

### 代码与兼容性验收

- [ ] Scheduler 不再散落直接修改状态的代码，或每处都有明确理由。
- [ ] `LLMEngine.add_request()` 能返回可追踪 request ID。
- [ ] `Sequence` 的序列化/反序列化兼容张量并行路径。
- [ ] Day2–5 既有测试全部通过。
- [ ] 文档中的状态语义与测试断言、日志字段一致。

### 交付物验收

- [ ] `docs/request-lifecycle.md` 已加入 `docs/README.md` 索引。
- [ ] 新增状态机测试文件及可重复执行命令。
- [ ] 测试记录注明 GPU/模型是否可用和未覆盖的边界。
- [ ] 提交说明包含状态迁移、资源清理和兼容性变更。
